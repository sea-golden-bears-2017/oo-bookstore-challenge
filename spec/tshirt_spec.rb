  ...
  describe "#appraise" do
    context "raises an error" do
      ...
      it "when condition is 'bad'" do
        expect{other_shirt.appraise(condition, price)}.toraise_error(Tshirt::InvalidCondition)
      end

      it "when condition is 'new'" do
        expect{shirt.appraise(condition_2, price_2)}.toraise_error(Tshirt::InvalidCondition)
      end
    end

    context "returns the discounted price based on the condition of theproduct" do
      let(:condition){other_shirt.condition}
      let(:price){other_shirt.price}
